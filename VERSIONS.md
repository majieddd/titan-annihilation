...
- **Every system contains every world.** The menu's choice is the home planet and the remaining slots take the other biomes in a seed-shuffled order, so Terran, Magma, Glacial, Arid and Barren are all present from the first launch. Systems now default to five planets.
- **Canyons.** A connected winding channel network is carved into every biome, with wide walkable floors, steep sides and branching tributaries, gated so it bites into land and fades to inlets at the coast. Mountain relief and fine grain are suppressed inside the floors so a channel crossing a range stays a route rather than inheriting its roughness. Depth, width and frequency are per biome: shallow ice fissures through to deep desert gorges.
- **Unit models move.** Road wheels roll at ground speed, walker legs swing and the hull bobs, helicopter rotors spin, and gun barrels recoil when they fire. It is all done inside the existing instanced draw call: each part carries an animation role and pivot baked at build time, and each unit passes a phase, a recoil value and a moving flag. Fitting this meant packing the per-vertex channels into two vec4s, because the instance matrix alone consumes four of the sixteen attribute slots the hardware guarantees.

## v3.6.1 — 2026-09-03 — Fix: the bundled build would not start
- v3.5.0 and v3.6.0 could not launch. The bundler hoists every external import into one shared scope, and v3.5.0 added `mergeVertices` alongside an existing `mergeGeometries` import of the same module, so the bundle declared that name twice and failed to parse. Nothing ran and the page sat on the loading spinner. The development page was unaffected because real modules each have their own scope, which is exactly why testing there missed it. Imports of the same module are now merged into one statement.
- The build now parses the bundled script before writing it, and refuses to write anything if it does not parse. Verified by injecting a syntax error: the build reports it, exits non-zero, and leaves the previous files untouched.

## v3.6.0 — 2026-09-03 — Reliquary and The Coil ported from the reference source; Diorama Ink corrected
- **Reliquary and The Coil are now ports, not impressions.** I read the shading code of both reference pages and brought over the actual model rather than approximating it: the lambert term is wrapped into 0..1 instead of clamped, so the dark side keeps a readable value and the terminator becomes a wide band; that band is quantised to four steps with the boundary offset per facet, which is what makes the result read as knife strokes instead of a shaded polyhedron; colour comes from a three-stop ramp whose deep stop is nearly pure hued shadow (violet, never black) and whose lit stop stays near the surface's own colour instead of marching toward white; on top of that sit hemispheric ambient, triplanar paint tooth, a hard-stepped specular standing in for the catch-light on a brush ridge, and a Fresnel rim in the palette's neon.
- The palette is derived the way the reference derives it, from one key colour and a shadow hue: shadow is the key dragged to the void hue and crushed, rim is the key at full saturation, ambient is a hued sky and ground pair either side of the shadow hue.
- Both styles also adopt the reference's presentation: flat-shaded faceted surfaces, a medium-poly world mesh, flat palette colour in place of photographic albedo, and their measured post values (exposure 0.94 and 0.88, saturation 1.24, contrast 1.10, vignette 0.54, grain 0.042, halftone 0.62, canvas 0.20, ink 0.95). The Coil keeps its own register: a band cap that stops the ground reaching the lit step, a tight glint instead of a wet sheen, a cobalt cast and a darker exposure.
- **Diorama Ink** is now what was asked for: the diorama look with hard black outlines and nothing else. The comic's crosshatching, halftone dots and paper grain are gone, and it is brighter across the board (stronger key, fill and environment light, higher exposure, neutral shadow tint, much lighter vignette).
