...
/** normal map derived from a cloud texture's alpha (treated as thickness) so the cloud deck shades like a volume */
export function cloudNormalTexture(cloudTex, strength = 6) {
  const src = cloudTex.image; const w = src.width, h = src.height;
  const rc = document.createElement('canvas'); rc.width = w; rc.height = h; const rctx = rc.getContext('2d'); rctx.drawImage(src, 0, 0);
  const a = rctx.getImageData(0, 0, w, h).data; const hgt = new Float32Array(w * h);
  for (let i = 0; i < w * h; i++) hgt[i] = a[i * 4 + 3] / 255;
  // small blur so the puffs get rounded shading instead of hard rims
  const bl = new Float32Array(w * h);
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) { let s = 0; for (let dy = -2; dy <= 2; dy++) for (let dx = -2; dx <= 2; dx++) s += hgt[((y + dy + h) % h) * w + ((x + dx + w) % w)]; bl[y * w + x] = s / 25; }
  const out = document.createElement('canvas'); out.width = w; out.height = h; const octx = out.getContext('2d'); const img = octx.createImageData(w, h); const d = img.data;
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
    const l = bl[y * w + (x - 1 + w) % w], r = bl[y * w + (x + 1) % w], u = bl[((y - 1 + h) % h) * w + x], dn = bl[((y + 1) % h) * w + x];
  ...
  const t = new THREE.CanvasTexture(out); t.wrapS = t.wrapT = THREE.RepeatWrapping; t.anisotropy = maxAniso; return t;
}
